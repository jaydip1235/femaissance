...
const Update = () => {
  ...
  useEffect(()=>{
    if(!localStorage.getItem("fmToken")) navigate('/login');
      ...
      axios.get('https://femaissance.herokuapp.com/api/user/getuser',config).then((res)=>{
  ...
  const updateData=async(e)=>{
    ...
    try {
      if(!username.trim()||!email.trim()||!phone.trim()||!bio.trim()) alert("Don't leave any field empty!");
        ...
        await axios.put('https://femaissance.herokuapp.com/api/user/updateprofile',{username,email,phone,bio,photo},config);
