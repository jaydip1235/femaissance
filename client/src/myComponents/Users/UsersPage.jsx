...
const UserPage=()=>{
    ...
    useEffect(()=>{
        ...
        axios.get('https://femaissance.herokuapp.com/api/user/getallusers',config).then((res)=>{
            setUsers(res.data);
            console.log(res.data);
            setLoading(false);
        }).catch((error)=>console.log(error));
        axios.get('https://femaissance.herokuapp.com/api/user/getuser',config).then((res)=>setCurrUser(res.data._id)).catch((err)=>console.log(err));
    },[])
                ...
                <Box sx={{ display: 'flex', alignItems: 'flex-end', justifyContent:'center',marginBottom:5 }}>
                    ...
                    onChange={async(e)=>{
                        try {
                            ...
                            const {data}=await axios.post('https://femaissance.herokuapp.com/api/user/getselecteduser',{field:e.target.value},config);
                            setUsers(data);
                        } catch (error) {
                            console.log(error);
                        }
