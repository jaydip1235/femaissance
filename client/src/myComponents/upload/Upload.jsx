...
const Upload = () => {
  ...
  useEffect(()=>{
    ...
    if(id){
      ...
      axios.get(`https://femaissance.herokuapp.com/api/posts/${id}`,config).then(({data})=>{
        console.log(data);
        setTitle(data.title);
        setText(data.body);
  ...
  const postData=async()=>{
    try {
      ...
      if(!body.trim()||!title.trim()||categories.length==0) alert("Don't leave any field empty!");
        ...
        if(!id){
          await axios.post(`https://femaissance.herokuapp.com/api/posts/create`,{title,body,photo,categories},config);
          alert("Post created!");
        }
        else{
          await axios.put(`https://femaissance.herokuapp.com/api/posts/${id}`,{title,body,photo,categories},config);
          alert("Post updated!");
        }
        navigate('/posts');
      }
