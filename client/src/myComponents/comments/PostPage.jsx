...
const PostPage=()=>{
    ...
    const getPost=async()=>{
        try {
            ...
            const {data}=await axios.get(`https://femaissance.herokuapp.com/api/posts/${_id}`,config);
            setPost(data);
            console.log(data);
        } catch (error) {
            console.log(error);
...
backgroundImage: `url("https://www.transparenttextures.com/patterns/shattered-dark.png")`}}>
            <div className='container d-flex'>
                ...
                <div className="row" style={{width:'100%'}}>
                    <PostData postedBy={post?post.postedBy.username:null} createdAt={post?timeDifference(new Date(),new Date(post.createdAt)):null} like={post?post.likes.length:null} body={post?post.body:null} title={post?post.title:null} photo={post?post.photo:null} />
                    <CommentGallery />
                </div>
            </div>
