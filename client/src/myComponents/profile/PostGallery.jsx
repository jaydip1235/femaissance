...
const PostGallery=()=>{
    ...
    const fetchData=async()=>{
        try{
            ...
            if(!userid){
                const {data}=await axios.get('https://femaissance.herokuapp.com/api/user/getuser',config);
                let res=await axios.get(`https://femaissance.herokuapp.com/api/user/posts/${data._id}`,config);
                setLoading(false);
                setPosts(res.data);
                console.log(data);
            }
            else{
            let res=await axios.get(`https://femaissance.herokuapp.com/api/user/posts/${userid}`,config);
            setLoading(false);
            setPosts(res.data);
            }
        }catch(error){
