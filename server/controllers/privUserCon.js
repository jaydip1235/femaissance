  ...
  exports.informUser=async(req,res)=>{
    try {
        ...
        sendEmail({
            email:user.email,
            subject:"New chat",
            message:`<h1>New chat is waiting for you!!!</h1><br><h4>Hi, ${user.username}. Click on the following button to join chat.</h4><br><a href="https://femaissance.netlify.app/chat/${roomId}/${userId}/${otherId}" target="_blank"><button style="color: white;background: purple;cursor: pointer;">Click here to join chat</button></a>`
        });
        res.status(200).json({message:"user informed!"});
    } catch (error) {
        console.log(error);
        res.status(500).json({ error: "Something went wrong!" });
    }
